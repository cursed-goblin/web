...
export default function Hero() {
  return (
    <section id="home" className="relative overflow-hidden pb-24 pt-32 sm:pt-40">
      {/* Blurred workspace backdrop + dark overlay */}
    ...
    >
      ...
      <div className="glass overflow-hidden rounded-2xl shadow-glow">
        ...
        <div className="flex items-center gap-2 border-b border-white/10 bg-white/5 px-4 py-3">
          ...
          <div className="ml-4 hidden flex-1 items-center rounded-md bg-ink-900/60 px-3 py-1.5 text-xs text-slate-400 sm:flex">
            https://your-shop.sitesprout.in
          </div>
        </div>

        {/* Dashboard body */}
        <div className="grid gap-4 p-5 text-left sm:grid-cols-3 sm:p-6">
          {/* Chart card */}
          ...
          <div className="glass flex flex-col justify-between rounded-xl p-4">
            <div>
              ...
              <p className="mt-1 text-lg font-bold text-white">Starter</p>
              <p className="text-sm text-slate-400">₹1,999 / year</p>
              <ul className="mt-3 space-y-1.5 text-xs text-slate-300">
                {["Free domain", "Custom email", "SSL security"].map((f) => (
                  <li key={f} className="flex items-center gap-1.5">
                    <Check className="h-3.5 w-3.5 text-accent-400" /> {f}
                  </li>
                ))}
              </ul>
            </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-white/5 bg-white/[0.03] px-4 py-3 sm:col-span-3 sm:pl-28">
            ...
            <div className="flex flex-wrap items-center gap-x-5 gap-y-2 opacity-70">
              {["Chai Point", "Sharma Kirana", "Bloom Florist", "Fit Gym", "Kraft Café"].map(
                (n) => (
      ...
      {/* Phone mockup peeking from bottom-left */}
      <motion.div
        initial={{ opacity: 0, x: -30, y: 30 }}
        animate={{ opacity: 1, x: 0, y: 0 }}
        transition={{ duration: 0.8, delay: 0.75 }}
        className="absolute -bottom-10 -left-4 hidden w-32 sm:block lg:-left-10"
      >
        <div className="glass rounded-[1.4rem] p-2 shadow-glow">
          <div className="rounded-[1rem] bg-gradient-to-b from-ink-700 to-ink-900 p-3">
...
function GrowthChart() {
  return (
    <svg viewBox="0 0 400 120" className="h-28 w-full" preserveAspectRatio="none">
      ...
      <path
        d="M0,95 C40,80 60,60 100,65 C140,70 160,40 200,38 C240,36 260,55 300,45 C340,36 360,18 400,12 L400,120 L0,120 Z"
        fill="url(#area)"
      />
      <path
        d="M0,95 C40,80 60,60 100,65 C140,70 160,40 200,38 C240,36 260,55 300,45 C340,36 360,18 400,12"
        fill="none"
        stroke="url(#line)"
        strokeWidth="3"
        strokeLinecap="round"
